...
namespace RoadSignReplacer
{
    ...
    public class UIFastList : UIComponent
    {
        ...
        /// <summary>
        /// Sets the selection to the named prop pack.
        /// If no prop pack is found is found, clears the selection.
        /// </summary>
        ...
        public void FindPack(string name)
        {
            // Iterate through the list to find the right one by name match.
            for (int i = 0; i < m_rowsData.m_size; i++)
            {
                if ((m_rowsData.m_buffer[i] as InternalPropPack).propPack.name.Equals(name))
                {
                    selectedIndex = i;
                    listPosition = i;
                    return;
